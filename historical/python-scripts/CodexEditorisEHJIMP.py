...
def execute_script(progress_var, complete_callback):
    try:
        ...
        # API URL
        url = 'https://www.ebi.ac.uk/europepmc/webservices/rest/search?query=JOURNAL%3A"Eur%20Heart%20J%20Imaging%20Methods%20Pract"&resultType=core&pageSize=1000&sort=P_PDATE_D%20desc&format=json'
        ...
        <head>
            ...
            <title>The Adimension Codex Editoris R4.2</title>
            <style>
                body {
        ...
        <body>
            ...
            <div class="cc-container"><br>S. Anwer © The Adimension | DEITY Framework 2025.<br><br></div>
...
def run_app():
    ...
    root = tk.Tk()
    root.title("The Adimension | Codex Editoris")
    root.geometry("400x300")  # Set the window size
    root.resizable(False, False)  # Disable resizing

    # Create a container frame to center content
    container = tk.Frame(root)
    container.pack(expand=True, fill="both")  # Center the container frame

    # Use grid to center widgets inside the frame
    container.grid_rowconfigure(0, weight=1)  # Space for logo
    container.grid_rowconfigure(1, weight=1)  # Space for subtitle
    container.grid_rowconfigure(2, weight=1)  # Space for start button
    container.grid_rowconfigure(3, weight=1)  # Space for progress bar
    container.grid_rowconfigure(4, weight=1)  # Space for save button
    container.grid_rowconfigure(5, weight=1)  # Space for footer
    container.grid_columnconfigure(0, weight=1)  # Center horizontally

    def start_script():
        # Disable the start button while running
        start_button.config(state=tk.DISABLED)
        # Run the script in a separate thread to prevent blocking the GUI
        threading.Thread(target=execute_script, args=(progress_var, on_complete)).start()

    def on_complete(output_file):
        # Enable the save button after execution completes
        save_button.config(state="normal", command=lambda: save_file(output_file))
    ...
    # Add a logo label
    logo_label = tk.Label(container, text="The Adimension | Codex Editoris", font=("Arial", 16), pady=10)
    logo_label.grid(row=0, column=0, pady=5)

    # Add a subtitle under the logo
    subtitle_label = tk.Label(container, text="EHJIMP", font=("Arial", 12), fg="gray")
    subtitle_label.grid(row=1, column=0, pady=5)

    # Add a start button
    start_button = Button(container, text="Run", command=start_script)
    start_button.grid(row=2, column=0, pady=10)

    # Add a progress bar (fixed position)
    progress_var = tk.DoubleVar()
    progress_bar = Progressbar(container, variable=progress_var, maximum=100, length=200)
    progress_bar.grid(row=3, column=0, pady=10)

    # Add a save button (initially disabled)
    save_button = Button(container, text="Save", state="disabled")
    save_button.grid(row=4, column=0, pady=10)

    # Add a footer at the bottom
    footer_line_1 = tk.Label(container, text="To AÏA © The Adimension 2025", font=("Arial", 10), fg="darkgray")
    footer_line_1.grid(row=5, column=0, pady=5)
